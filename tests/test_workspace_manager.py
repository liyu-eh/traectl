...
class TestManageMcp:
    ...
    def test_list_returns_standard_response(self):
        with tempfile.TemporaryDirectory() as tmp:
            resp = self.wm.manage_mcp(tmp, "list")
            assert isinstance(resp, StandardResponse)
            assert resp.code == 0
            assert resp.result["action"] == "list"
    ...
    def test_add_mcp_server(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            resp = self.wm.manage_mcp(tmp, "add", server_name="test-server", server_config=config)
            assert isinstance(resp, StandardResponse)
            assert resp.code == 0
            assert resp.result["status"] == "added"

    def test_add_mcp_without_name_returns_error(self):
        with tempfile.TemporaryDirectory() as tmp:
            resp = self.wm.manage_mcp(tmp, "add", server_config={"command": "x"})
            assert resp.code == -1

    def test_add_mcp_without_config_returns_error(self):
        with tempfile.TemporaryDirectory() as tmp:
            resp = self.wm.manage_mcp(tmp, "add", server_name="test")
            assert resp.code == -1

    def test_update_mcp_server(self):
        with tempfile.TemporaryDirectory() as tmp:
            config = {"command": "npx", "args": ["-y", "server"]}
            self.wm.manage_mcp(tmp, "add", server_name="srv", server_config=config)
            resp = self.wm.manage_mcp(tmp, "update", server_name="srv", server_config={"args": ["new"]})
            assert resp.code == 0
            assert resp.result["status"] == "updated"
            # merged config
            assert resp.result["config"]["command"] == "npx"
            assert resp.result["config"]["args"] == ["new"]

    def test_update_nonexistent_returns_error(self):
        with tempfile.TemporaryDirectory() as tmp:
            resp = self.wm.manage_mcp(tmp, "update", server_name="nope", server_config={"x": 1})
            assert resp.code == -1

    def test_remove_mcp_server(self):
        with tempfile.TemporaryDirectory() as tmp:
            self.wm.manage_mcp(tmp, "add", server_name="srv", server_config={"command": "x"})
            resp = self.wm.manage_mcp(tmp, "remove", server_name="srv")
            assert resp.code == 0
            assert resp.result["status"] == "removed"

    def test_remove_nonexistent(self):
        with tempfile.TemporaryDirectory() as tmp:
            resp = self.wm.manage_mcp(tmp, "remove", server_name="nope")
            assert resp.result["status"] == "not_found"

    def test_remove_without_name_returns_error(self):
        with tempfile.TemporaryDirectory() as tmp:
            resp = self.wm.manage_mcp(tmp, "remove")
            assert resp.code == -1

    def test_unknown_action_returns_error(self):
        with tempfile.TemporaryDirectory() as tmp:
            resp = self.wm.manage_mcp(tmp, "invalid")
            assert resp.code == -1

    def test_add_existing_server_returns_updated(self):
        with tempfile.TemporaryDirectory() as tmp:
            config1 = {"command": "old"}
            self.wm.manage_mcp(tmp, "add", server_name="srv", server_config=config1)
            config2 = {"command": "new"}
            resp = self.wm.manage_mcp(tmp, "add", server_name="srv", server_config=config2)
            assert resp.result["status"] == "updated"


# ── WorkspaceManager._load_mcp_json / _save_mcp_json ──────────
...
class TestMcpJsonIO:
    ...
    def test_load_nonexistent_returns_empty(self):
        with tempfile.TemporaryDirectory() as tmp:
            data = self.wm._load_mcp_json(tmp)
            assert data == {"servers": {}}

    def test_save_and_load_roundtrip(self):
        with tempfile.TemporaryDirectory() as tmp:
            data = {"servers": {"srv": {"command": "npx", "args": ["-y", "s"]}}}
            self.wm._save_mcp_json(tmp, data)
            loaded = self.wm._load_mcp_json(tmp)
            assert loaded == data

    def test_load_corrupted_json_returns_empty(self):
        with tempfile.TemporaryDirectory() as tmp:
            mcp_path = Path(tmp) / ".trae" / "mcp.json"
...
# ── .trae/mcp.json 读写 (high-level API) ──────────────────────

class TestMcpJson:
    """测试 .trae/mcp.json 读写。"""

    def test_load_empty(self, tmp_path):
        wm = WorkspaceManager(config_path=tmp_path / "cfg.yaml")
        servers = wm._get_mcp_servers(str(tmp_path))
        assert servers == {}

    def test_add_server(self, tmp_path):
        wm = WorkspaceManager(config_path=tmp_path / "cfg.yaml")
        wm._set_mcp_server(str(tmp_path), "test-server", {"command": "npx", "args": ["-y", "test"]})
        servers = wm._get_mcp_servers(str(tmp_path))
    ...
    def test_update_server(self, tmp_path):
        wm = WorkspaceManager(config_path=tmp_path / "cfg.yaml")
        wm._set_mcp_server(str(tmp_path), "test", {"command": "v1"})
        wm._set_mcp_server(str(tmp_path), "test", {"command": "v2"})
        servers = wm._get_mcp_servers(str(tmp_path))
        assert servers["test"]["command"] == "v2"

    def test_delete_server(self, tmp_path):
        wm = WorkspaceManager(config_path=tmp_path / "cfg.yaml")
        wm._set_mcp_server(str(tmp_path), "test", {"command": "npx"})
        removed = wm._delete_mcp_server(str(tmp_path), "test")
        assert removed is True
        assert wm._get_mcp_servers(str(tmp_path)) == {}

    def test_delete_nonexistent(self, tmp_path):
        wm = WorkspaceManager(config_path=tmp_path / "cfg.yaml")
        removed = wm._delete_mcp_server(str(tmp_path), "nope")
        assert removed is False

    def test_mcp_file_format(self, tmp_path):
        wm = WorkspaceManager(config_path=tmp_path / "cfg.yaml")
        wm._set_mcp_server(str(tmp_path), "fs", {"command": "npx", "args": ["-y", "@mcp/fs"]})
        mcp_path = tmp_path / ".trae" / "mcp.json"
